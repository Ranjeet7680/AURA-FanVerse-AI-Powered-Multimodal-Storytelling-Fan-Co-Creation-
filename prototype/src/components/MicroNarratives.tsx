  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-purple-900/50 via-indigo-900/40 to-slate-900/90 border border-purple-500/30 rounded-3xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl backdrop-blur-md"
      >
        <div>
          <div className="flex items-center space-x-2 text-purple-400 text-xs font-semibold uppercase tracking-wider mb-1 font-mono">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <span className="px-3 py-1.5 rounded-xl bg-purple-500/20 border border-purple-400/30 text-purple-300 text-xs font-medium flex items-center gap-1.5 font-mono">
            <Zap className="w-3.5 h-3.5 text-yellow-400 animate-pulse" />
            {t('shorts.edge_ingestion', 'Edge Ingestion: 38s')}
          </span>
          {/* Format Switcher: 9:16 Shorts vs 16:9 Long Videos */}
          <div className="flex items-center p-1 rounded-2xl bg-[#120d22] border border-cyan-500/30 shadow-inner">
            <button
              onClick={() => {
                soundFX.playClick();
      ...
      {/* Animate Format Switch */}
      <AnimatePresence mode="wait">
        {mediaFormat === 'long' ? (
          /* LONG MATCH VIDEO EXPERIENCE */
          <motion.div
            key="long-format"
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-md active:scale-95 shimmer-effect"
                    >
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {selectedLongVideo.keyMoments.map((km, idx) => (
                      <div
                        key={idx}
                        className="p-3 rounded-xl bg-[#1a142c] border border-slate-800 text-xs flex items-center justify-between"
                      >
                        <span className="text-slate-300 font-medium">{km.title}</span>
                  ...
                  return (
                    <motion.div
                      ...
                      className={`cursor-pointer rounded-2xl overflow-hidden border transition-all duration-200 group flex flex-col justify-between ${
                        isSelected
                          ? 'bg-[#1e1738] border-cyan-400 shadow-xl shadow-cyan-900/30 ring-1 ring-cyan-400'
                    ...
                    >
                      <div className="relative aspect-video w-full overflow-hidden bg-black">
                        ...
                        <span className="absolute top-2 left-2 text-[10px] bg-cyan-600/90 px-2 py-0.5 rounded text-white font-bold font-mono">
                          {vid.category}
                        </span>
                      </div>
            ...
            <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none text-xs">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-full font-bold whitespace-nowrap transition-all active:scale-95 ${
                    activeCategory === cat
                      ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-600/30'
                      : 'bg-[#18112c] text-slate-400 hover:text-slate-200 border border-slate-800/80 hover:border-slate-700'
                  }`}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-5 flex flex-col items-center space-y-4">
                ...
                <div className="flex items-center justify-between w-full max-w-[340px] px-1 text-xs">
                  <div className="flex items-center gap-1 bg-[#161129] p-1 rounded-2xl border border-slate-800">
                    <button
                      onClick={() => {
                        soundFX.playClick();
                        setViewMode('embed');
                ...
                <div className="relative w-full max-w-[340px] aspect-[9/16] rounded-3xl overflow-hidden shadow-2xl border-4 border-purple-500/30 bg-black flex flex-col justify-between neon-border-dynamic">
                  {viewMode === 'embed' ? (
                    <iframe
                    ...
                    <div className="relative w-full h-full flex flex-col justify-between p-4">
                      ...
                      <div className="relative z-10 flex items-center justify-between">
                        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-pink-600/90 text-white shadow-lg backdrop-blur-sm font-mono">
                          {selectedReel.badge}
                        </span>
                ...
                {/* Quick AI Voice Commentary Bar below Video with Equalizer Animation */}
                <div className="w-full max-w-[340px] p-3 rounded-2xl bg-[#1d1433] border border-purple-500/30 flex items-center justify-between gap-3 text-xs shadow-lg">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {isSpeaking && (
                      <div className="flex items-center gap-0.5 h-4">
                        <div className="w-1 bg-cyan-400 wave-bar-1 rounded-full" />
                        <div className="w-1 bg-pink-400 wave-bar-2 rounded-full" />
                        <div className="w-1 bg-purple-400 wave-bar-3 rounded-full" />
                      </div>
                    )}
                    <button
                      onClick={() => handlePlayVoice(currentCaption)}
                      className={`p-2 rounded-xl flex items-center gap-1.5 text-xs font-bold transition-all active:scale-95 ${
                        isSpeaking
              ...
              <div className="lg:col-span-7 space-y-4">
                    ...
                    return (
                      <motion.div
                        ...
                        className={`cursor-pointer rounded-2xl p-3 border transition-all duration-200 text-left relative overflow-hidden group ${
                          isSelected
                            ? 'bg-purple-950/50 border-purple-500 shadow-xl shadow-purple-900/30 ring-1 ring-purple-400'
                            : 'bg-[#16102a] border-slate-800/80 hover:border-slate-700 hover:bg-[#1a1332]'
                        }`}
                      >
                        <div className="flex space-x-3">
                          <div className="relative w-20 h-28 rounded-xl overflow-hidden flex-shrink-0 bg-slate-800">
                            ...
                            {reel.category && (
                              <span className="absolute top-1 left-1 text-[9px] bg-purple-600/90 px-1 rounded text-white font-bold font-mono">
                                {reel.category}
                              </span>
                            )}
                          </div>
                ...
                <div className="bg-[#16102a] border border-purple-500/20 rounded-2xl p-5 space-y-3 mt-4 shadow-xl">
                  <div className="flex items-center justify-between">
